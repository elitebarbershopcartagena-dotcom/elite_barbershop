...
const BladeDB = (() => {
  ...
  function _blockFromDB(row) {
    return { id: row.id, barberId: row.barber_id, fecha: row.fecha,
             horaInicio: row.hora_inicio, horaFin: row.hora_fin,
             motivo: row.motivo || '', creadoEn: row.created_at };
  }

  function _dayFromDB(row) {
    return { id: row.id, barberId: row.barber_id, fecha: row.fecha, disponible: row.disponible };
  }
  ...
  const DEF_SERVICES = [
    { id: 1, name: 'Corte Clásico',       price: 25000, duracion: 30, desc: 'Corte tradicional con tijera y máquina.', icon: '✂️' },
    { id: 2, name: 'Afeitado Ritual',     price: 35000, duracion: 45, desc: 'Experiencia completa con navaja abierta.', icon: '🪒' },
    { id: 3, name: 'Diseño de Barba',     price: 20000, duracion: 20, desc: 'Perfilado y sculpting personalizado.', icon: '💈' },
    { id: 4, name: 'Combo Premium',       price: 70000, duracion: 90, desc: 'Corte + barba + afeitado + tratamiento.', icon: '⚡' },
    { id: 5, name: 'Fade Degradado',      price: 30000, duracion: 40, desc: 'Técnica de degradado de alta precisión.', icon: '🔥' },
    { id: 6, name: 'Tratamiento Capilar', price: 28000, duracion: 35, desc: 'Nutrición profunda con productos naturales.', icon: '🌿' },
  ];
  const DEF_INVENTORY = [
    ...
    { id: 7, name: 'Gel Fijador Negro',    cat: 'Fijadores',     qty: 12, price: 18000, min: 5,  desc: 'Fijación extrema sin residuos.' },
  ...
  /* ══════════════════════════════════════════════════════════════
     § 6  MOTOR DE DISPONIBILIDAD (síncrono — usa cache)
     ══════════════════════════════════════════════════════════════ */
  ...
  async function crearCita(datos) {
    ...
    // Regla: 1 pendiente por teléfono (online)
    if (origen === 'online') {
      let existente = null;
      if (_online()) {
        try {
          ...
          existente = _ls.get('cache_appts', []).find(
            a => a.telefono === telefono && (a.estado === 'pending' || a.estado === 'pendiente'));
        }
      } else {
        existente = _ls.get('cache_appts', []).find(
          a => a.telefono === telefono && (a.estado === 'pending' || a.estado === 'pendiente'));
  ...
  async function confirmarCita(id) {
    ...
    if (_online()) {
      try {
        const { error } = await _sb.from('appointments').update({ status: 'confirmed' }).eq('id', id);
        if (error) throw error;
        return { ok: true };
      } catch (err) {
    ...
    if (_online()) {
      try {
        const { error } = await _sb
          .from('barber_days').upsert([payload], { onConflict: 'barber_id,fecha' });
        if (error) throw error;
        return;
      } catch (err) {
  ...
  async function crearBloqueo(datos) {
    ...
    if (_online()) {
      try {
        const { data, error } = await _sb.from('blocks').insert([payload]).select().single();
        if (error) throw error;
      ...
      try {
        if (item.action === 'insert') {
          ...
          if (item.table === 'appointments' && item.localId) {
            ...
            if (idx > -1) { cache[idx] = _apptFromDB(data); _ls.set('cache_appts', cache); }
          }
          synced++;
        } else if (item.action === 'update') {
          const { error } = await _sb.from(item.table).update(item.payload).eq('id', item.id);
          if (error) throw error;
          synced++;
        } else if (item.action === 'upsert') {
          const { error } = await _sb
            .from(item.table).upsert([item.payload], { onConflict: 'barber_id,fecha' });
          if (error) throw error;
          synced++;
        }
      } catch (err) {
        console.error('[sync] fallo:', item, err);
        failed++;
        remaining.push(item);
      }
    }
  ...
  return {
    ...
    addCut:   (cut) => { const cuts = _ls.get('cuts', []); cuts.push({ ...cut, id: _tempId() }); _ls.set('cuts', cuts); },
    ...
    addAccount:  (username, data) => { const e = _ls.get('extra_accounts', {}); e[username] = data; _ls.set('extra_accounts', e); },
    ...
    deleteAccount: (username) => { const e = _ls.get('extra_accounts', {}); delete e[username]; _ls.set('extra_accounts', e); },
